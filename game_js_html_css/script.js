window.addEventListener("load", function () {
  ...
  /**
   * Caveat: Can not reproduce two or more audio at the same time,
   * The strategy is reproduce one audio at the time, and if it has
   * to be reproduced again it is rewined
   */
  ...
  class Shield {
    ...
    draw(context) {
      context.drawImage(
        ...
        this.game.player.x,
        this.game.player.y,
        this.width, //las two arguments if need rescaling
        this.height 
      );
    }
    reset() {
  ...
  class Projectile {
    constructor(game, x, y) {
      ...
      this.width = 36.25;
      this.height = 20;
      this.speed = Math.random() * 0.2 + 2.8; // Between 2.8 and 3 animation per frame
    ...
    update(deltaTime) {
      this.x += this.speed;
      if(this.timer > this.interval) { //Animate the next frame
  ...
  class Particle {
    constructor(game, x, y) {
      ...
      this.markedForDeletion = false;
      this.angle = 0;
      this.va = Math.random() * 0.2 - 0.1; // Rotation speed radiant
      this.bounced = 0; // How many times bounce
    ...
    /**
     * Anything done between save and restore will just affect the code.The drawing code in between
     */
    draw(context) {
      context.save(); // Save the current canvas state. 
      //translate to the object we want to rotate
      ...
      context.drawImage(
        ...
        this.size, 
        this.size);
      context.restore(); // Look for associated save method and will rest all kind of settings
    }
  }
  class Player {
    constructor(game) {
    ...
    update(deltaTime) {
      ...
      this.y += this.speedY;
      this.x += this.speedX; // TODO ver si tengo que remover esto si no se mueve en X
      //Vertical Boundaries
  ...
  class Explosion {
    constructor(game, x, y) {
      ...
      this.fps = 30;
      this.timer = 0; // Timer and interval are two helper variable to control the fps of the fire and dust
      this.interval = 1000/this.fps;
      this.markedForDeletion = false;
      this.maxFrame = 8;
    ...
    update(deltaTime) {
      this.x -= this.game.speed; //In order to move with the camera
      //In order to slow down the animation:
      if(this.timer > this.interval) {
        this.frameX++;
        this.timer = 0;
      } else {
  ...
  class Game {
    constructor(width, height) {
      ...
      this.shield = new Shield(this); // After the player because it is used in the shield constructor
      this.keys = [];
      this.enemies = [];
      this.particles = [];
      this.explosions = [];
    ...
    update(deltaTime) {
      ...
      this.enemies.forEach(enemy => {
        ...
        this.player.projectiles.forEach(projectiles => {
          if(this.checkCollition(projectiles, enemy)) {
            enemy.lives--;
    ...
    addExplosion(enemy) {
      ...
      if (randomize < 0.5) {
        ...
        this,
        enemy.x + enemy.width * 0.5, // To appear in the center of the enemy
